...
2. **編輯節點屬性**：點擊畫布上的節點，右側會出現 **Property Panel**。你可以在此修改 Label、資料庫類型（SQL/NoSQL）、讀寫比例或指定產品（如 Cassandra）。
3. **建立連線**：點擊並拖曳組件邊緣的點（Handle）來連接不同節點。
4. **語法分析**：點擊上方 **Analyze** 按鈕（或使用快捷鍵 `Cmd/Ctrl + A`），前端會將拓撲結構發送至後端進行驗證。
5. **檢查警告**：若架構不合規範，畫面上會顯示後端回傳的警告訊息。
