...
| 編號 | 規則名稱 | 觸發條件 / 說明 |
| :--- | :--- | :--- |
| 1 | **單點故障檢查 (SPOF)** | 負載均衡器 (LB) 下游僅連接 1 個服務節點時觸發警告。 |
| 2 | **資料庫選型建議** | 當使用 SQL 資料庫且寫入比例超過 50% (`readWriteRatio < 0.5`) 時，提示檢查效能。 |
| 3 | **垂直切分提醒** | 當拓撲中存在 2 個（含）以上的資料庫節點時，提示可能需要考慮垂直切分。 |
| 4 | **快取一致性檢查** | 當服務同時連接至快取 (Cache) 與資料庫 (Database) 時，提示需處理數據一致性。 |
| 5 | **CAP 定理提醒** | 當使用 NoSQL 資料庫（如 Cassandra, DynamoDB 等）時，自動標記適用的 CAP 特性。 |
